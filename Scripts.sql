...
select Top 1 FORMAT(date, 'MMMM'),r.r_name, count(o.r_id) total_order
...
--offset 0 row fetch next 1 row only     -- work same as top 1
...
select r_name, count(*) as total_repeated_customers
from cte
group by r_name
...
with loyal as (
select r.r_name as restaurant ,u.name,  count(o.user_id) as Visits
from users u join orders o on u.user_id = o.user_id join restaurants r on o.r_id = r.r_id
group by u.name, r.r_name
)
select restaurant, name as LoyalCustomer, Visits as max_visits
from loyal l1
where Visits = (select max(Visits) from loyal l2 where l2.restaurant = l1.restaurant)
